...
func LogStackTraceAndExit(opDetails ...string) {
	if p := recover(); p != nil {
		logAndExit(p, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.MakeFields(getMessage(opDetails)))
	}
}
...
func LogStackTraceAndContinue(opDetails ...string) {
	if p := recover(); p != nil {
		logAndContinue(p, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.MakeFields(getMessage(opDetails)))
	}
}
...
func HandlePanicWithError(p interface{}, err error, opDetails ...string) error {
	...
	if p != nil {
		...
		handleWithError(perr, err, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.MakeFields(getMessage(opDetails)))
	}
...
func AnnotateMessageWithStackTraceAndContinue(m interface{}) {
	if p := recover(); p != nil {
		logAndContinue(p, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.ConvertToComposer(level.Critical, m))
	}
}
...
func AnnotateMessageWithStackTraceAndExit(m interface{}) {
	if p := recover(); p != nil {
		logAndExit(p, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.ConvertToComposer(level.Critical, m))
	}
}
...
func AnnotateMessageWithPanicError(p interface{}, err error, m interface{}) error {
	...
	if p != nil {
		...
		handleWithError(perr, err, grip.NewLogger(grip.GetGlobalLogger().GetSender()), message.ConvertToComposer(level.Critical, m))
	}

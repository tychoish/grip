...
// instance.
func Context(ctx context.Context) Logger { return ContextLoger(ctx, string(defaultContextKey)) }

// WithContextLogger attaches a logger with a specific name to the
// context to the context. Your package should wrap this and use
...
// is returned.
func ContextLoger(ctx context.Context, name string) Logger {
	if ctx == nil {
		return std
	}
